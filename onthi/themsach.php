				...
				<form id="main-form" action="xulythemsach.php" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<select class="form-control custom-select" name="cbochude">
							...
							<?php
								$rsChuDe = getAllChuDe();
								while($rowChuDe = mysqli_fetch_assoc($rsChuDe)) {
							?>
							<option value="<?php echo $rowChuDe["MaCD"]; ?>"><?php echo $rowChuDe["Tenchude"]; ?></option>
							<?php } ?>
						</select>
					</div>
					<div class="form-group">
						...
						<select class="form-control custom-select" name="cbonhaxb">
							...
							<?php
								$rsNSX = getAllNSX();
								while($rowNSX = mysqli_fetch_assoc($rsNSX)) {
							?>
							<option value="<?php echo $rowNSX["MaNXB"]; ?>"><?php echo $rowNSX["TenNXB"]; ?></option>
							<?php } ?>
						</select>
					</div> 
					<div class="form-group">
